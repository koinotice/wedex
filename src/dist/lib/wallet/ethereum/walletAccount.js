...
const LoopringWallet = wallets_json_1.default.find(wallet => utils_1.trimAll(wallet.name).toLowerCase() === "loopringwallet");
...
function privateKeytoAddress(privateKey) {
    ...
    return formatter_1.formatAddress(ethereumjs_util_1.privateToAddress(privateKey)).toLocaleLowerCase();
}
...
class MetaMaskAccount extends WalletAccount {
    ...
    signMessage(message) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield MetaMask.signMessage(this.web3, this.account, message);
            if (!result["error"]) {
                return result["result"];
            }
            else {
    ...
    signEthereumTx(rawTx) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield MetaMask.signEthereumTx(this.web3, this.account, rawTx);
            if (!result["error"]) {
                return result["result"];
            }
            else {
